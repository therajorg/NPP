...
*/	
	var tmpR = "?tmpr="+Math.floor(Math.random()*80000);
	
	
    // menu button NPP Provider EN url
    var domNPPProvEnOpenFile = document.getElementById('nppprovenOpenFile');
    domNPPProvEnOpenFile.onclick = function (event) {
	  app.retriever.loadUrl('../npp_prov_en.json'+tmpR, app.openCallback);
      event.stopPropagation();
      event.preventDefault();
	  document.getElementById('openedfilenameID').innerHTML = "npp_prov_en.json";
    };
    // menu button NPP Plan EN url
    var domNPPPlanEnOpenFile = document.getElementById('nppplanenOpenFile');
    domNPPPlanEnOpenFile.onclick = function (event) {
	  app.retriever.loadUrl('../npp_plan_en.json'+tmpR, app.openCallback);
      event.stopPropagation();
      event.preventDefault();
	  document.getElementById('openedfilenameID').innerHTML = "npp_plan_en.json";
    };
    // menu button NPP Provider ES url
    var domNPPProvEsOpenFile = document.getElementById('nppprovesOpenFile');
    domNPPProvEsOpenFile.onclick = function (event) {
	  app.retriever.loadUrl('../npp_prov_es.json'+tmpR, app.openCallback);
      event.stopPropagation();
      event.preventDefault();
	  document.getElementById('openedfilenameID').innerHTML = "npp_prov_es.json";
    };
    // menu button NPP Plan Es url
    var domNPPPlanEsOpenFile = document.getElementById('nppplanesOpenFile');
    domNPPPlanEsOpenFile.onclick = function (event) {
	  app.retriever.loadUrl('../npp_plan_es.json'+tmpR, app.openCallback);
      event.stopPropagation();
      event.preventDefault();
	  document.getElementById('openedfilenameID').innerHTML = "npp_plan_es.json";
    };
    ...
    domSave.onclick = app.saveFileToServer;
...
app.saveFileToServer = function () {
  ...
  app.lastChanged = undefined;
	...
	var tmpfname = $("#openedfilenameID").html();
	
	if(tmpfname == "npp_prov_en.json" || tmpfname == "npp_plan_en.json" || tmpfname == "npp_prov_es.json" || tmpfname == "npp_plan_es.json") {
	} else {
		alert("Error: Invalid file name!");
		return;
	}

  // save the text from the code editor
  // TODO: show a 'saving...' notification
  var jsonObject = codeEditor.getText();
	
	$.ajax({
		type : "POST",
		url : "savejsontofile.php",
		/*dataType : 'json',*/
		data : {
			fname: tmpfname,
			json : jsonObject
	 ...
	 .done(function(retdata) {
		 if(retdata == "SUCCESS") {
 	     	alert("Success: Saved the file");
		 } else {
	      	app.notify.showError(retdata);
		 }
	})
...
app.clearFile = function () {
  document.getElementById('openedfilenameID').innerHTML = "";
